...
export async function getPokemons(
	next: string = `${BASE_URL}/pokemon/?limit=${POKEMONS_PER_PAGE}&offset=0`,
): Promise<PokemonList> {
...
export async function getPokemon(key:string,url: string) {
	const res = await fetch(url)
	const element: PokemonCard = await res.json()
	const imageUrl = element.sprites.other[
		"official-artwork"
